...
class TestAdvertisementRouter:
    @pytest.mark.parametrize("payload,status_code,is_violation,probability", [
        (
            {"seller_id": 1, "name": "Test", "description": "desc",
             "category": 1, "images_qty": 5, "item_id": 10, "is_verified_seller": True},
            200, True, 0.8,
        ),
        (
            {"seller_id": 1, "name": "Test", "description": "desc",
    ...
    async def test_predict(self, async_client: AsyncClient, app, payload,
                           status_code, is_violation, probability, mock_account):
    ...
    async def test_simple_predict(self, async_client: AsyncClient, app, payload,
                                  status_code, exception, mock_account):
        app.dependency_overrides[get_current_account] = lambda: mock_account
        ...
        if exception:
            ...
            mock_service.simple_predict.return_value = PredictionResult(
    ...
    async def test_close_advertisement(self, async_client: AsyncClient, app, payload,
                                       close_success, status_code, mock_account):
        app.dependency_overrides[get_current_account] = lambda: mock_account

        mock_service = AsyncMock()
        mock_service.close_advertisement.return_value = ActionStatus(success=close_success)
...
class TestAuthRouter:
    ...
    async def test_login(self, async_client: AsyncClient, app, login, password,
                         service_side_effect, expected_status):
        mock_service = AsyncMock()
        if service_side_effect:
            mock_service.login.side_effect = service_side_effect
        else:
            mock_service.login.return_value = TokenPairResponse(
                user_token="access_token", refresh_token="refresh_token"
            )
    ...
    async def test_refresh(self, async_client: AsyncClient, app, cookies,
                           service_side_effect, expected_status):
        mock_service = AsyncMock()
        if service_side_effect:
            mock_service.refresh_token.side_effect = service_side_effect
        else:
            mock_service.refresh_token.return_value = TokenPairResponse(
                user_token="new_access_token", refresh_token="new_refresh_token"
            )
...
class TestModerationRouter:
    ...
    async def test_async_predict(self, async_client: AsyncClient, app, payload,
                                 service_side_effect, expected_status, mock_account):
        app.dependency_overrides[get_current_account] = lambda: mock_account
        ...
        if service_side_effect:
            ...
            mock_service.start_moderation.return_value = MagicMock(
    ...
    async def test_get_moderation_result(self, async_client: AsyncClient, app, task_id,
                                         service_side_effect, expected_status, mock_account):
        app.dependency_overrides[get_current_account] = lambda: mock_account
        ...
        if service_side_effect:
            ...
            mock_service.get_moderation_status.return_value = MagicMock(
