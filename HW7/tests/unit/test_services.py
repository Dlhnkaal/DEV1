...
class TestAdvertisementMLServicePredict:
    """Проверяем логику predict: is_violation=True/False, модель недоступна."""
    ...
    async def test_predict_model_none_raises_model_not_ready(self):
        service = AdvertisementMLService()
        service._model = None

        with patch.object(service, '_load_model', side_effect=Exception("load failed")):
    ...
    def test_load_model_failure_sets_model_to_none(self):
        ...
        with patch('services.advertisement.os.path.exists', return_value=True), \
             patch('services.advertisement.load_model', side_effect=Exception("corrupt file")):
    ...
    def test_load_model_trains_when_file_missing(self):
        ...
        with patch('services.advertisement.os.path.exists', return_value=False), \
             patch('services.advertisement.train_model', return_value=mock_model) as mock_train, \
             patch('services.advertisement.save_model'):
            service._load_model()
...
class TestAdvertisementMLServiceSimplePredict:
    ...
    async def test_close_advertisement(self, item_id):
        ...
        service.advertisement_repo.close = AsyncMock(return_value=ActionStatus(success=True))
...
class TestAuthService:
    ...
    @pytest.mark.parametrize("token,payload,account,expected", [
        ("good",
         ...
         MagicMock(id=1, is_blocked=False), True),
        ("expired",
         {"user_id": 1, "expired_at": (datetime.now() - timedelta(hours=1)).isoformat()},
        ...
        ("blocked",
         {"user_id": 1, "expired_at": (datetime.now() + timedelta(hours=1)).isoformat()},
         MagicMock(id=1, is_blocked=True), UnauthorizedError),
    ])
