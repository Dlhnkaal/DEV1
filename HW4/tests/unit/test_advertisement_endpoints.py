...
class TestPredictEndpoint:
    ...
    def test_predict_type_errors(self, client, field, invalid_data):
        payload = {"seller_id": 1, "is_verified_seller": True, "item_id": 1, "name": "Test", "description": "desc", "category": 1, "images_qty": 5}
...
class TestSimplePredictEndpoint:
    ...
    @pytest.mark.parametrize("missing_field", ["item_id"])
    def test_simple_predict_missing_fields(self, client, missing_field):
        full_data = {"item_id": 3}
        del full_data[missing_field]
        response = client.post("/advertisement/simple_predict", json=full_data)
        assert response.status_code == status.HTTP_422_UNPROCESSABLE_CONTENT
    ...
    @pytest.mark.parametrize("field,invalid_data", [("item_id","-1")])
    def test_simple_predict_constraints(self, client, field, invalid_data):
        payload = {"item_id": 1}
        payload[field] = invalid_data
        response = client.post("/advertisement/simple_predict", json=invalid_data)
        assert response.status_code == status.HTTP_422_UNPROCESSABLE_CONTENT
