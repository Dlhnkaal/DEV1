...
from repositories.advertisement import AdvertisementPostgresStorage, AdvertisementRepository, AdvertisementRedisStorage
...
@pytest.fixture
def mock_ad_redis_storage():
    redis = AsyncMock(spec=AdvertisementRedisStorage)
...
@pytest.fixture
def mock_moderation_repo():
    repo = AsyncMock(spec=ModerationRepository)
    repo.delete_from_cache_by_item_id = AsyncMock()
    repo.get_task_ids_by_item_id = AsyncMock()
    return repo
...
@pytest.fixture
def mock_ad_repo(mock_ad_storage, mock_ad_redis_storage, mock_moderation_repo):
...
def mock_ml_service(mock_ad_repo):
    ...
    service.close_advertisement = AsyncMock(return_value=True)
