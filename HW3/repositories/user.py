...
class UserPostgresStorage:
    async def create(self, name: str, email: str, is_verified_seller: bool) -> Mapping[str, Any]:
        logger.info("Creating user name=%s, email=%s", name, email)
        query = """
        ...
        async with get_pg_connection() as connection:
            row = await connection.fetchrow(
                query, name, email, is_verified_seller)
            if row:
                return dict(row)
    ...
    async def get_by_email(self, email: str) -> Optional[Mapping[str, Any]]:
...
class UserRepository:
    ...
    async def get_by_email(self, email: str) -> Optional[UserInDB]:
        raw_user = await self.storage.get_by_email(email)
        if raw_user:
            return UserInDB(**raw_user)
        return None
